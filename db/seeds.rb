...
#       language: 'English',
#       length: 400,
#       user_input: "Write a children's bedtime story about a #{Faker::Creature::Animal.name} that goes on an adventure.",
#       age_group: age_groups.sample,
#       genre: genres.sample,
#       user_id: user.id
#     )
...
#     - Summary of the bedtime story not longer than one sentence in #{prompt[:language]}
...
#     response_string = OpenaiService.new(prompt_sent).call
#     story_text = JSON.parse(response_string)

#     # Generate story picture using OpenAI API
#     picture_prompt = "Create a cover photo for a children's bedtime story. The style should be fantasy and based on the following summary: #{story_text[:summary]}"
...
#       body: story_text[:body],
#       summary: story_text[:summary],
#       public: true,
#       follow_up_summary: story_text[:follow_up_summary],
#       prompt_id: prompt.id
#     )
